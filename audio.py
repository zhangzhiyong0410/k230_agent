...
def record_audio(filename, duration=None, btn=None):
    # 降低采样率和声道数，以减小文件大小
    RATE = 16000           # 较低采样率，语音识别足够
    CHANNELS = 1           # 单声道
    CHUNK = RATE // 25     # 根据采样率重新计算 chunk
    FORMAT = paInt16       # 16bit 精度
    ...
    try:
        ...
        # 按键控制模式：btn 不为 None 时启用
        if btn is not None:
            print("wait button press to start record...")
            # 等待按键按下（低电平 0 为按下）
            while btn.value() != 0:
                if exit_check():
                    break
            ...
            print("start record (button pressed)...")
            ...
            while True:
                ...
                # 按键松开（高电平 1）结束录音
                if btn.value() == 1:
                    print("button released, stop record...")
                    break

                # 如同时设置 duration，则作为最长录音时长上限
                if duration is not None:
                    if time.ticks_diff(time.ticks_ms(), start_ms) >= int(duration * 1000):
                        print("reach max duration, stop record...")
                        break
            ...
            # 固定时长模式：保持原有行为
            if duration is None:
                raise ValueError("duration 不能为空，当 btn 未提供时必须指定录音时长")
...
def audio_recorder(filename, duration=None, btn=None):
